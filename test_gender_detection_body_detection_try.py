            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if face_crop.shape[0] > 10 and face_crop.shape[1] > 10:
                        ...
                        cv2.putText(im0, gender_label, (int(xyxy[0]), int(xyxy[1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 2.0,
                                    (0, 255, 0), 3)
                        # body_model.predict(im0, conf=0.2, classes=0, show=True, save=True, show_labels=False, show_conf=False)


                    if save_img or view_img:  # Add bbox to image
                        label = f'{names[int(cls)]} {conf:.2f}'
                        # plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=5)
                        plot_one_box(xyxy, im0, color=colors[int(cls)], line_thickness=16)

                body_model.predict(im0, conf=0.6, classes=0, show=True, show_labels=False, show_conf=False)
