...
#     #debug lines start
#     print("printing the types of xyxy and im0s")
#     print(type(xyxy))
#     print(type(im))
#     print("printing the values of xyxy and im0s")
#     print("xyxy         \n",xyxy)
#     print("im0s         \n",im)
#     #stop the program
#     # exit()
...
def save_one_box(xyxy, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False):
    ...
    # Access the shape using the length of the first channel (height)
    shape = len(im[0])
    clip_coords(xyxy, (shape, shape))

    # Convert the tensor indices to integers
    y1, y2, x1, x2 = int(xyxy[0, 1]), int(xyxy[0, 3]), int(xyxy[0, 0]), int(xyxy[0, 2])

    # Check if the indices are within bounds
    if 0 <= y1 < y2 <= shape and 0 <= x1 < x2 <= shape:
        crop = im[0][y1:y2, x1:x2]
        # Check if crop is empty or has invalid dimensions
        if crop is not None and crop.shape[0] > 0 and crop.shape[1] > 0:
            # Save the cropped image
            cv2.imwrite(str(increment_path(file, mkdir=True).with_suffix('.jpg')), crop if BGR else crop[..., ::-1])
        else:
            print("Crop is empty or has invalid dimensions. Image not saved.")
            print(crop.shape)
            
    else:
        print("Indices are out of bounds. Image not saved.")
        print("y1, y2, x1, x2")
        print(y1, y2, x1, x2)
    # exit()


# runnign good but not saving immediately
# def save_one_box(xyxy, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False, region=(560, 400)):
#     # Save an image crop as {file} with crop size multiplied by {gain} and padded by {pad} pixels
#     xyxy = torch.tensor(xyxy).view(-1, 4)
#     b = xyxy2xywh(xyxy)  # boxes
#     if square:
...
#     region_x1, region_y1, region_x2, region_y2 = shape - region[0], shape - region[1], shape, shape

#     # Clip coordinates to the specified region
#     clip_coords(xyxy, (region_x2, region_y2))

#     # Convert the tensor indices to integers
#     y1, y2, x1, x2 = int(xyxy[0, 1]), int(xyxy[0, 3]), int(xyxy[0, 0]), int(xyxy[0, 2])

#     # Check if the indices are within bounds
#     if region_x1 <= x1 < x2 <= region_x2 and region_y1 <= y1 < y2 <= region_y2:
#         crop = im[0][y1:y2, x1:x2]
#         # Check if crop is empty or has invalid dimensions
#         if crop is not None and crop.shape[0] > 0 and crop.shape[1] > 0:
#             # Save the cropped image
